...
module Auth
  ...
  def generate_auth_token(ttl = ENV["AUTH_TOKEN_TTL"], refresh_ttl = ENV["AUTH_TOKEN_TTL"])
    ...
    auth_token.token = ::AuthTokenService.encode({ user_id: self.id }, ttl)
  ...
  def invalidate_auth_token
    ...
    auth_token.token = ::AuthTokenService.encode({ user_id: self.id }, exp)
  ...
  def update_tracked_fields(env)
    ...
    self.last_sign_in_at = self.current_sign_in_at || new_current
    self.current_sign_in_at = new_current

    new_current = env["REMOTE_ADDR"]
    self.last_sign_in_ip = self.current_sign_in_ip || new_current
